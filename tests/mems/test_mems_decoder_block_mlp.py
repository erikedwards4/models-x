"""
Pytest function for mems/mems_decoder_block_mlp.py.
"""
import pytest
import jax
...
# mems_decoder_block_mlp.MeMsDecoderBlockMLP
@pytest.mark.parametrize("d_model", (768, ))
@pytest.mark.parametrize("dtype", (jnp.float32, ))
def test_mems_decoder_block_mlp(d_model, dtype):
    """
    Pytest mems_decoder_block_mlp.MeMsDecoderBlockMLP.
    """
    # Start
    print("")
    ...
    # Get mdl
    mlp = MeMsDecoderBlockMLP.from_config(cfg=cfg)
    assert isinstance(mlp, MeMsDecoderBlockMLP)
    assert callable(mlp)
