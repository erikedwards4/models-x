"""
Pytest function for mems/mems_decoder_block_attn.py.
"""
import pytest
import jax
...
# mems_decoder_block_attn.MeMsDecoderBlockAttn
@pytest.mark.parametrize("d_model", (768, ))
@pytest.mark.parametrize("dtype", (jnp.float32, ))
def test_mems_decoder_block_attn(d_model, dtype):
    """
    Pytest mems_decoder_block_attn.MeMsDecoderBlockAttn.
    """
    # Start
    print("")
    ...
    # Get mdl
    mha = MeMsDecoderBlockAttn.from_config(cfg=cfg)
    assert isinstance(mha, MeMsDecoderBlockAttn)
    assert callable(mha)
