"""
Memory Mosaics Transformer Decoder stage,
which is the main section with 1-12 decoder layers.

See mems_decoder_block.py for the MHA (multi-head attention).
This Decoder is simply a stack of these blocks, e.g.
12 layers (repeated blocks) for the Memory Mosaics small version.

The input is the word embeddings from MeMsStem with dim d_model.
...
from models_x.mems.mems_decoder_block import MeMsDecoderBlock
...
class MeMsDecoder():
    ...
    def from_config(cls: type[Self],
        ...
        blocks: list[MeMsDecoderBlock] = []
        for _ in range(nblocks):
            block = MeMsDecoderBlock.from_config(cfg=cfg, **kwargs)
            blocks.append(block)
